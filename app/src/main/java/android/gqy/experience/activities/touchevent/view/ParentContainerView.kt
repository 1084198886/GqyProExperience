...
class ParentContainerView : LinearLayout {
     ...
     *   后续的事件也会直接交给child处理；如果child未拦截，且child.onTouchEvent返回false，则交由parent onTouchEvent处理，
     *   如果parent.onTouchEvent处理针对的ACTION_DOWN返回true，则后续事件都由parent处理；否则parent交给上层的parent(最终到达Activity.onTouchEvent)处理.
     *
     *   4,如果parent.onInterceptTouchEvent没有拦截ACTION_DOWN事件，被child拦截，后续的ACITON_MOVE事件仍然会经过parent的 dispatchTouchEvent，onInterceptTouchEvent
     *
     *   5,OnTouchListener.onTouch返回false，parent.onInterceptTouchEvent  ACTION_DOWN返回false，默认返回true，child的onTouchEvent 针对ACTION_DOWN返回true，
     *   默认返回false。触摸事件开始时，ACTION_DOWN事件由child处理，紧跟的第一个ACTION_MOVE事件被parent拦截时，parent.onTouchEvent并不会执行，而是出发了
     *   child.onTouchEvent方法，产生ACITON_CANCEL事件，后续的move，up事件会直接由parent.onTouchEvent处理了。
     */
